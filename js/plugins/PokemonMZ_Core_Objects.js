...
PokemonMZ_Game_Pokemon.prototype.itemEffect = function(item, ext1) {
    let currentHp; let mHp; let recovered; let recoverPercent; let nextHp;
    ...
    case "recoverHpPercentCureStatus":
        ...
        recovered = nextHp - currentHp;
        recoverPercent = 100 * recovered / mHp;
        return {"effect":"recoverAndCureStatus","status":item.pkmz_data.status,"value":recovered, "percentValue":recoverPercent};
    case "increaseLevel":
    ...
    case "battlePatkUpUser":
        if (this._stageModifiers.patk < 6) {
            ...
            if (this._stageModifiers.patk > 6) { this._stageModifiers.patk = 6; }
            return {"effect":"patkUp","animation":item.pkmz_data.battleAnimation, "success":true};
        } else {
           return {"effect":"patkUp","success":false};
        }
    ...
    case "battleAccUpUser":
        if (this._stageModifiers.acc < 6) {
            this._stageModifiers.acc += item.pkmz_data.stage;
            if (this._stageModifiers.acc > 6) { this._stageModifiers.acc = 6; }
            return {"effect":"accUp","animation":item.pkmz_data.battleAnimation, "success":true};
        } else {
...
PokemonMZ_Game_Pokemon.prototype.isBerserkable = function() {
    // Cannot berserk fainted pokemon
    if (this.isFainted()) {
        return false;
    }
    return true;
...
PokemonMZ_Game_Pokemon.prototype.berserk = function(minTurns, maxTurns, moveIndex, force) {
    if (this.isBerserkable() || force) {
...
PokemonMZ_Game_Pokemon.prototype.reduceBerserkTurn = function() {
    this._turnsBerserk--;
    if (this._turnsBerserk <= 0) {
        // When berserk moves ends, the pokemon becomes confused
        this.unBerserk();
        this.confuse(true)
    }
};
...
PokemonMZ_Game_Pokemon.prototype.unBerserk = function() {
    if (this.isBerserk()) {
        this._isBerserk = false;
        this._turnsBerserk = 0;
        this._berserkMoveIndex = -1;
...
PokemonMZ_Game_Action.prototype.calculateItem = function() { //TODO 
    ...
    switch (effect.effect) {
        ...
        case "accUp":
            if (effect.success) {
                this._resultSteps.push(["hitAnimation", effect.animation, this.userBattleSprite(), null, this.side()])
                this._resultSteps.push(["autotext","accuracyRose",this.side()])
            } else {
                this._resultSteps.push(["autotext","statusNothing",this.side()])
            }
            break;
...
PokemonMZ_Game_Action.prototype.calculateMove = function() { //TODO
    ...
    // If berserk, displays message of thrashing about
    if (this._user.isBerserk()) {
        this._resultSteps.push(["autotext","thrashing",this.side()])
    }
...
PokemonMZ_Game_Action.prototype.calculateMoveAttack = function() { 
    ...
    if (hit || this._moveExecutedHits > 0) {
        if (this._moveData.target == "opponent") {
            ...
            this._resultSteps.push(["hitAnimation", this._moveData.animationHit, this._user._battleSprite, this._user._battleSprite, this.side()]);
        }
...
PokemonMZ_Game_Action.prototype.calculateMoveStatus = function() { 
    ...
    if (hit) {
        ...
        if (effectResults.success) {
            if (this._moveData.target == "opponent") {
                this._resultSteps.splice(nextResultIndex, 0, ["hitAnimation", this._moveData.animationHit, this._user._battleSprite, this._opponent._battleSprite, this.side()]);
            } else if (this._moveData.target == "user") {
                this._resultSteps.splice(nextResultIndex, 0, ["hitAnimation", this._moveData.animationHit, this._user._battleSprite, this._user._battleSprite, this.side()]);
            };
        }
    } else {
...
PokemonMZ_Game_Action.prototype.calculateMoveBide = function() { 
    ...
    if (!this._user.isBiding()) {
        ...
        if (damage > 0) {
            ...
            this._resultSteps.push(["hitAnimation", bideAnimation, this._user._battleSprite, this._opponent._battleSprite, this.side()]);
...
PokemonMZ_Game_Action.prototype.effect_spcUpUser = function(battleData, effect, effectResults) {
    if (this._opponent.hp() - battleData.damageDealt <= 0) {
        // No effect if target will faint
        return effectResults;
    }
    const randomNumber = Math.randomInt(100)
    if (PokemonMZ.debugLog) {
        console.log({"PokemonMZ_Game_Action.effect_spcUpUser > ":{
            "chance":effect.percentChance, "randomNumber":randomNumber, "stageBefore":this._user._stageModifiers.satk}
        })
    }
    if (randomNumber < effect.percentChance) {
...
PokemonMZ_Game_Action.prototype.effect_berserkUser = function(battleData, effect, effectResults) {
    if (!this._user.isBerserk()) {
